...
class GUI():
    def __init__(self, master):
        ...
        self.label = Label(text=":PASSWORD GENERATOR:", anchor=N, fg=('BLACK'), bg='#808A87',
                           font='arial 20 bold underline')
        self.label.grid(row=0, column=1)

        self.blank_label1 = Label(text="" ,bg = 'grey')
        self.blank_label1.grid(row=1, column=0, )



        self.blank_label2 = Label(text="" ,bg = 'grey')
        self.blank_label2.grid(row=3, column=0, columnspan=2)

        self.user = Label(text="Enter User Name: ", font='times 15 bold', bg='#808A87', fg='BLACK')
        self.user.grid(row=4, column=0)

        self.textfield = Entry(textvariable=self.n_username, font='times 15', bd=6, relief='ridge',bg = 'grey79')
        self.textfield.grid(row=4, column=1)
        self.textfield.focus_set()

        self.blank_label3 = Label(text="" ,bg = 'grey')
        self.blank_label3.grid(row=5, column=3)

        self.length = Label(text="Enter Password Length: ", font='times 15 bold', bg='#808A87', fg='BLACK')
        self.length.grid(row=6, column=0)

        self.length_textfield = Entry(textvariable=self.n_passwordlen, font='times 15', bd=6, relief='ridge',bg = 'grey79')
        self.length_textfield.grid(row=6, column=1)

        self.blank_label4 = Label(text=" ",bg = 'grey')
        self.blank_label4.grid(row=7, column=5)

        self.generated_password = Label(text="Generated Password: ", font='times 15 bold', bg='#808A87', fg='black')
        ...
        self.generated_password_textfield = Entry(textvariable=self.n_generatedpassword, font='times 15', bd=6,
                                                  relief='ridge', fg='#DC143C',bg = 'GREY79')
        ...
        self.generate = Button(text="GENERATE PASSWORD", bd=3, relief='solid', padx=1, pady=1, font='Verdana", 15 bold',
                               fg='black', bg='#808A87', command=self.generate_pass)
        self.generate.grid(row=11, column=1)

        self.blank_label5 = Label(text="" ,bg = 'grey')
        ...
        self.accept = Button(text="ACCEPT", bd=3, relief='solid', padx=1, pady=1, font='Helvetica 15 bold italic',
                             fg='grey72', bg='black', command=self.accept_fields)
        self.accept.grid(row=13, column=1)

        self.blank_label1 = Label(text="" ,bg = 'grey')
        self.blank_label1.grid(row=14, column=2)

        self.reset = Button(text="RESET", bd=3, relief='solid', padx=1, pady=1, font='Helvetica 15 bold italic',
                            fg='grey72', bg='black', command=self.reset_fields)
    ...
    def generate_pass(self):
        ...
        if length < 5:
            messagebox.showerror("Error", "Password must be atleast 5 characters long")
